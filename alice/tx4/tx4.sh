...
TX4_uns="$( tx_build 1 "" "${TX4_in}" "${TX4_out}" 0 | tr -d ' \n' )"
TX4_mid="${TX4_uns}01000000"
TX4_sha256="$( sha256 ${TX4_mid} )"
TX4_z="$( hash256 ${TX4_mid} )"

TX4_sig_A1="$( signder "${A1_hex}" "${TX4_sha256}" "${TX4_z}" | tr -d ' \n' )"

TX4_sig_C0="$( signder "${C0_hex}" "${TX4_sha256}" "${TX4_z}" | tr -d ' \n' )"

TX4_in_sig="$( tx_mkin_serialize ${TX4_inpoint[@]} "$((2**32-1))" "0 @${TX4_sig_A1} @${TX4_sig_C0} @${spk_2_A1C0_2_hex}" )"

TX4_fin="$( tx_build 1 "" "${TX4_in_sig}" "${TX4_out}" 0 | tr -d ' \n' )"
