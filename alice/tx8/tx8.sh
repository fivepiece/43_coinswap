...
TX8_uns="$( tx_build 1 "" "${TX8_in}" "${TX8_out}" 0 | tr -d ' \n' )"
TX8_mid="${TX8_uns}01000000"
TX8_sha256="$( sha256 ${TX8_mid} )"
TX8_z="$( hash256 ${TX8_mid} )"

TX8_sig_A5="$( signder "${A5_hex}" "${TX8_sha256}" "${TX8_z}" | tr -d ' \n' )"

TX8_in_sig="$( tx_mkin_serialize ${TX8_inpoint[@]} "$((2**32-1))" "@${TX8_sig_A5} @${AX_sec} 1 @${spk_HXA5_L1C5_hex}" )"

TX8_fin="$( tx_build 1 "" "${TX8_in_sig}" "${TX8_out}" 0 | tr -d ' \n' )"
