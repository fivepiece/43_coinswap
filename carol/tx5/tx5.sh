...
TX5_uns="$( tx_build 1 "" "${TX5_in}" "${TX5_out}" 0 | tr -d ' \n' )"
TX5_mid="${TX5_uns}01000000"
TX5_sha256="$( sha256 ${TX5_mid} )"
TX5_z="$( hash256 ${TX5_mid} )"

TX5_sig_A3="$( signder "${A3_hex}" "${TX5_sha256}" "${TX5_z}" | tr -d ' \n' )"

TX5_sig_C4="$( signder "${C4_hex}" "${TX5_sha256}" "${TX5_z}" | tr -d ' \n' )"

TX5_in_sig="$( tx_mkin_serialize ${TX5_inpoint[@]} "$((2**32-1))" "0 @${TX5_sig_A3} @${TX5_sig_C4} @${spk_2_A3C4_2_hex}" )"

TX5_fin="$( tx_build 1 "" "${TX5_in_sig}" "${TX5_out}" 0 | tr -d ' \n' )"
